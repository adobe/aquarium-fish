...
var (
	flags  flag.FlagSet
	target = flags.String("target", "go", "target lang to generate for")
)
...
{{- range $serviceName, $methods := .Services }}
  {{ trimSuffix $serviceName "Service" }} = "{{ $serviceName }}",
{{- end }}
}
...
{{- range $serviceName, $methods := .Services }}
// {{ $serviceName }} service constants
export enum Perm{{ trimSuffix $serviceName "Service" }} {
{{- range $idx, $method := $methods }}
{{- if ne $method "" }}
  {{ $method }} = "{{ $method }}",
{{- end }}
...
func generatePermissionFiles(plugin *protogen.Plugin, grpcServices map[string][]string, grpcRoles map[string][]Permission) error {
	// Generate gRPC-specific permissions
	if err := generateServiceMethodConstants(plugin, grpcServices, "grpc"); err != nil {
		return err
	}

	// Generate combined permissions
	return generateCombinedPermissions(plugin, grpcRoles)
}

func generateServiceMethodConstants(plugin *protogen.Plugin, services map[string][]string, suffix string) error {
	...
	switch *target {
	case "go":
		err := serviceMethodConstantsTmplGo.Execute(g, map[string]any{
			"Services":     services,
		...
		outFile = "permissions_" + suffix + ".gen.go"
	case "ts":
		tmpl, err := template.New("service_method_constants").Funcs(template.FuncMap{
			"trimSuffix": strings.TrimSuffix,
		}).Parse(serviceMethodConstantsTmplTs)
		if err != nil {
			return fmt.Errorf("Error parsing gRPC template: %v", err)
		}
		err = tmpl.Execute(g, map[string]any{
			"Services":     services,
			"AuthExcluded": authExcluded,
			"RbacExcluded": rbacExcluded,
		})
		...
		outFile = "permissions_" + suffix + ".ts"
	default:
		return fmt.Errorf("Target %s is not supported", *target)
	}

	genFile := plugin.NewGeneratedFile(outFile, "")
	if _, err := genFile.Write(g.Bytes()); err != nil {
		return fmt.Errorf("Error writing service/method constants file %s: %v", outFile, err)
	}

	return nil
}

func generateCombinedPermissions(plugin *protogen.Plugin, grpcRoles map[string][]Permission) error {
	if *target == "ts" {
		// We don't need combined permissions in TS client
		return nil
	}
	...
	outFile := "permissions.gen.go"
	genFile := plugin.NewGeneratedFile(outFile, "")
	if _, err := genFile.Write(g.Bytes()); err != nil {
		return fmt.Errorf("Error writing combined permissions file %s: %v", outFile, err)
	}
...
func main() {
	...
	protogen.Options{
		...
		return generatePermissionFiles(plugin, grpcServices, grpcRoles)
	})
}
