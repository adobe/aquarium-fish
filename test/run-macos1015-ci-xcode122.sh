...
if [ -z "$label_id" ]; then
    label_id=$(curl -u "admin:$token" -k -X POST -H 'Content-Type: application/json' -d '{"name":"'$label'", "version":'$version', "driver":"vmx",
        "definition": {
            "image": "macos1015-ci-xcode122",
            "images": {
                "macos1015": "https://artifact-storage/aquarium/image/macos1015-VERSION/macos1015-VERSION.tar.xz",
                "macos1015-ci": "https://artifact-storage/aquarium/image/macos1015-ci-VERSION/macos1015-ci-VERSION.tar.xz",
                "macos1015-ci-xcode122": "https://artifact-storage/aquarium/image/macos1015-ci-xcode122-VERSION/macos1015-ci-xcode122-VERSION.tar.xz"
            },
            "requirements": {
                "cpu": 14,
                "ram": 12,
