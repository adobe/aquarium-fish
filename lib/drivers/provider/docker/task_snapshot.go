...
func (t *TaskSnapshot) Execute() (result []byte, err error) {
	logger := log.WithFunc("docker", "TaskSnapshot").With("provider.name", t.driver.name)
	// docker commit 19808ab8fc07 container_name:snapshot-250807.174600

	if t.ApplicationTask == nil {
		logger.Error("Invalid application task", "task", t.ApplicationTask)
		return []byte(`{"error":"internal: invalid application task"}`), fmt.Errorf("DOCKER: %s: Invalid application task: %#v", t.driver.name, t.ApplicationTask)
	}
	...
	snapshotName := cName + ":snapshot" + time.Now().UTC().Format("-060102.150405")
	snapshotID, _, err := util.RunAndLogRetry("docker", 3, 10*time.Minute, nil, t.driver.cfg.DockerPath, "commit", cID, snapshotName)
	if err != nil {
	...
	// TODO: Capture the state of image in case full = true, snapshot only mounted disks if full = false
	// TODO: Stop the instance if t.ApplicationTask.When == typesv2.ApplicationState_DEALLOCATE

	logger.Info("Created snapshots for instance", "snapshots", snapshotName, "snapshots_id", snapshotID)

	return json.Marshal(map[string]any{"snapshots": []string{snapshotID}, "snapshot_names": []string{snapshotName}})
}
