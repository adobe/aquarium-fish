	...
	for _, ra := range all {
		if ra.Username == username && ra.Password == hash {
			if err = d.GateProxySSHAccessDelete(ra.Uid); err != nil {
				...
				log.WithFunc("proxyssh", "GateProxySSHAccessSingleUsePasswordHash").Error("Unable to remove GateProxySSHAccess", "access_uid", ra.Uid.String(), "err", err)
			}
	...
	for _, ra := range all {
		if ra.Username == username && ra.Key == key {
			if err = d.GateProxySSHAccessDelete(ra.Uid); err != nil {
				...
				// in the event that our deletion failed (but nothing else), the single
				// use connection ultimately gets rejected.
				log.WithFunc("proxyssh", "GateProxySSHAccessSingleUseKey").Error("Unable to remove GateProxySSHAccess", "access_uid", ra.Uid.String(), "err", err)
			}
			return &ra, d.GateProxySSHAccessDelete(ra.Uid)
		}
	}
