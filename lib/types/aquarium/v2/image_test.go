...
func Test_image_validate(t *testing.T) {
	t.Run("good_url", func(t *testing.T) {
		...
		if err := image.Validate(); err != nil {
			t.Fatalf(`image.Validate() = %q, unexpected error: %v`, url, err)
		}
		if image.GetName() != "test-image-ci" {
			t.Fatalf(`image.Validate() = %q, name is not equal the expected one: %v`, url, image.Name)
		}
		if image.GetVersion() != "20230210.190425_ff1cd1cf" {
			t.Fatalf(`image.Validate() = %q, version is not equal the expected one: %v`, url, image.Version)
		}
	})

	t.Run("url_empty", func(t *testing.T) {
		image := Image{
			Url: func() *string { val := ""; return &val }(),
		}

		if err := image.Validate(); err == nil || err.Error() != "Image: Url or Name should be provided" {
			t.Fatalf(`image.Validate() = %q, Url or Name error was expected, but error is: %v`, *image.Url, err)
		}
	})

	t.Run("img_empty", func(t *testing.T) {
		image := Image{}

		if err := image.Validate(); err == nil || err.Error() != "Image: Url or Name should be provided" {
			t.Fatalf(`image.Validate() = nil, Url or Name error was expected, but error is: %v`, err)
		}
	})
	...
	t.Run("bad_url_schema", func(t *testing.T) {
		...
		if err := image.Validate(); err == nil || err.Error() != `Image: Url schema is not supported: "ftp://tst"` {
			t.Fatalf(`image.Validate() = %q, URL error expected, but incorrect was returned: %v`, url, err)
		}
	})
...
func Test_image_downloadunpack(t *testing.T) {
	...
	t.Run("good", func(t *testing.T) {
		...
		if err := image.DownloadUnpack(t.TempDir(), "user", "password"); err != nil {
			t.Fatalf(`image.DownloadUnpack() = %q, unexpected error: %v`, url, err)
		}
	})
	...
	t.Run("bad_url", func(t *testing.T) {
		...
		if err == nil || err.Error() != `Image: Unable to download file "`+server.URL+`/not/existing/artifact-version.tar.xz": 404 Not Found` {
			t.Fatalf(`image.DownloadUnpack() = %q, error expected, but incorrect was returned: %v`, url, err)
		}
	})
