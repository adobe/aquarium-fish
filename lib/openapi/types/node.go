...
var NodePingDuplicationErr = errors.New("Fish Node: Unable to join the Aquarium cluster due to " +
	fmt.Sprintf("the node with the same name pinged the cluster less then %dsec ago", NODE_PING_DELAY*2))
...
func (n *Node) Init(node_address, cert_path string) error {
	...
	if n.Pubkey == nil {
		...
		if bytes.Compare(*n.Pubkey, pubkey_der) != 0 {
			return errors.New("Fish Node: The pubkey was changed for Node, that's not supported")
		}
	}
