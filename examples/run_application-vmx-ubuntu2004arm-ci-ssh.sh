...
response="$(curl -s -u "admin:$token" -X POST -d "{\"application_uid\":\"$app_id\"}" -k "https://$hostport/grpc/aquarium.v2.ApplicationService/GetResource")"
...
curl -u "admin:$token" -X POST -d "{\"application_resource_uid\":\"$resource_UID\"}" -k "https://$hostport/grpc/aquarium.v2.GateProxySSHAccess/GetResourceAccess"

echo 'Now you can `ssh -p PORT admin@0.0.0.0`, where PORT by default is 1222 - and use password from response to connect'
